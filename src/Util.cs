...
namespace Plivo
{
    ...
    public class Util
    {
        ...
        public class XPlivoSignature
        {
            ...
            {
                var isMatch = false;
                foreach (KeyValuePair<string, object> kvp in plivoHttpParams.OrderBy(key => key.Key, StringComparer.Ordinal))
        ...
        public static string PascalCaseToSnakeCase(string name)
        {
            return string.Concat(
                (char.ToLowerInvariant(name[0]) + name.Substring(1)).Select(
